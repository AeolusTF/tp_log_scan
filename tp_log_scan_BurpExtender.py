...
class BurpExtender(IBurpExtender, IContextMenuFactory, ITab, IHttpListener, IMessageEditorController, AbstractTableModel):
    def registerExtenderCallbacks(self, callbacks):
        ...
        self._callbacks.setExtensionName('tp_log_scan')
    ...
    def loadMenus(self):
        self.menus = []
        self.mainMenu = JMenu("tp_log_scan")
    ...
    def eventHandler(self, x):
        ...
        for message in self.messages:
            ...
            tp_log_scan  = tp_log_scan(url, version, year, month, day)
            log_path = tp_log_scan.checkLogPath()
            if not log_path:
                print("[-] {} can't get log file! ".format(url))
                self._callbacks.issueAlert("{} can't get log file".format(url))
                return

            filename_list = tp_log_scan.genFileName()
